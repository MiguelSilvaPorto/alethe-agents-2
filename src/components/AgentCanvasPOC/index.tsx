      ...
      <DragOverlay dropAnimation={null}>
        {draggingAgent ? (
          <AgentChip
            ...
            summary={
              AGENT_LIBRARY.find((t) => t.name === draggingAgent)?.summary
            }
            ghost
...
function AgentCanvasInner() {
  ...
  const [hooksSettingsPath, setHooksSettingsPath] = useState<string | null>(
    null,
  );
  ...
  const dispatchToAgent = useCallback(
    (payload: { agent?: string; task?: string; mode?: string }) => {
      ...
      const liveWorkers = codexWorkersRef.current.filter(
        (w) => w.exitedCode === null,
      ).length;
      ...
      spawnAgentWorker(
        agent,
        title,
        interactive ? { open: true } : { task: safe },
      );
    },
    [spawnAgentWorker, t],
  );
  ...
  useEffect(() => {
    ...
    const onDown = (e: PointerEvent) => {
      if (
        usageAnchorRef.current &&
        !usageAnchorRef.current.contains(e.target as Node)
      ) {
        setUsageOpen(false);
      }
    };
  ...
  useLayoutEffect(() => {
    const recompute = () => {
      ...
      const taskEdges: Edge[] = Object.values(tasks).flatMap((task) => {
        ...
        const ownerEl = task.owner
          ? cardRefs.current.get(`teammate:${task.owner}`)
          : null;
  ...
  useEffect(() => {
    if (!session) return;
    const tick = () => {
      void useNodeCostStore
        .getState()
        .refresh(useAgentCanvasStore.getState().nodes);
  ...
  const routingSavings = estimateRoutingSavings(
    nodeCosts,
    leadCost?.model ?? null,
    pricing,
  );
  ...
  const budgetRatio =
    budgetUsd && budgetUsd > 0 ? sessionCostUsd / budgetUsd : 0;
  const budgetWarn = budgetUsd != null && budgetUsd > 0 && budgetRatio >= 0.8;
  const budgetCrit =
    budgetUsd != null && budgetUsd > 0 && sessionCostUsd >= budgetUsd;
  ...
  const renderCard = (node: AgentNode) => {
    ...
    return (
      ...
      >
        ...
        {node.prompt ? (
          <div className={styles.cardPrompt}>{node.prompt}</div>
        ) : null}
        {node.feed.length > 0 ? (
        ...
        {cost ? (
          <div className={styles.cardCost}>
            ...
            {cost.cost_usd != null ? (
              <span
                className={`${styles.cardCostUsd} ${costClassFor(cost.cost_usd)}`}
              >
    ...
    <div className={styles.layout}>
      <div className={styles.main}>
        ...
        >
          <header className={styles.topBar}>
            ...
            <div className={styles.topRight}>
              ...
              {hasCost ? (
                ...
                >
                  <Coins size={12} />
                  <span className={costClassFor(sessionCostUsd)}>
          ...
          <div
            className={
              paletteOpen ? styles.agentPalette : styles.agentPaletteCollapsed
            }
            data-no-pan
            style={{
          ...
          >
            ...
            <div
              className={
                planeDragOver
                  ? `${styles.plane} ${styles.planeDropTarget}`
                  : styles.plane
              }
              ref={(el) => {
                planeRef.current = el;
            ...
            >
                ...
                {leadCost ? (
                  <div className={styles.cardCost}>
                    {shortModel(leadCost.model) ? (
                      <span className={styles.cardCostModel}>
                        {shortModel(leadCost.model)}
                      </span>
                    ) : null}
                    <span className={styles.cardCostTokens}>
                    ...
                    {leadCost.cost_usd != null ? (
                      <span
                        className={`${styles.cardCostUsd} ${costClassFor(leadCost.cost_usd)}`}
                      >
              ...
              <span
                className={
                  running > 0 ? styles.planeDotActive : styles.planeDot
                }
              />
            </div>
              ...
              <div className={styles.codexRow}>
                {codexWorkers.map((w) => (
                  ...
                  >
                    ...
                    {w.result ? (
                      <div className={styles.codexResult}>{w.result}</div>
                    ) : null}
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        <span className={styles.agentGroupCount}>
                          {cards.length}
                        </span>
                      </div>
                  ...
                  {taskList.map((task) => (
                    ...
                    >
                      <div className={styles.taskNodeHead}>
                        ...
                        />
                        <span className={styles.taskNodeSubject}>
                          {task.subject}
                        </span>
                      </div>
        ...
        <div className={styles.terminalDock}>
          <div className={styles.terminalHeader}>
            ...
            <button
              ...
              className={
                economyOn
                  ? `${styles.clearButton} ${styles.economyOn}`
                  : styles.clearButton
              }
      ...
      {(() => {
        ...
        return (
          <div
            className={styles.codexOverlay}
            onClick={() => setExpandedCodexId(null)}
          >
            <div
              className={styles.codexModal}
              ...
              <div className={styles.terminalHost}>
                <XTermView
                  ...
                  onExit={(code) => {
                    ...
                    setCodexWorkers((prev) =>
                      prev.map((x) =>
                        x.ptyId === w.ptyId ? { ...x, exitedCode: code } : x,
                      ),
                    );
                  }}
                />
