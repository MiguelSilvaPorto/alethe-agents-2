...
async function writePtyChunked(
  ...
  for (let index = 0; index < text.length; index += PASTE_CHUNK_SIZE) {
    ...
    await new Promise((resolve) =>
      window.setTimeout(resolve, PASTE_CHUNK_DELAY_MS),
    );
  }
...
export function XTermView({
  ...
  const openMarkdownInGrid = useCallback(
    (target: string) => {
      ...
      useProjectsStore
        .getState()
        .createMarkdownPane(projectId, { filePath: target });
    },
    [projectId],
  );
  ...
  useLayoutEffect(() => {
    ...
    linkProviderDisposable = terminal.registerLinkProvider({
      provideLinks: (bufferLineNumber, callback) => {
        const logicalLine = getLogicalTerminalLine(
    ...
    if (command !== "opencode") {
      ...
      try {
        ...
        webglAddon.onContextLoss(() => {
          ...
          // e tenta carregar o Canvas addon como fallback.
          webglAddon?.dispose();
          webglAddon = null;
          try {
    ...
    const onWheel = (event: WheelEvent) => {
      ...
      if (
        !shouldScrollHostScrollback(terminal.buffer.active.type, event.shiftKey)
      )
        return;
    ...
    const onResizeRequest = (event: Event) => {
      const targetPtyId = (event as CustomEvent<{ ptyId?: string }>).detail
        ?.ptyId;
    ...
    async function start() {
      try {
        ...
        if (command && cwd)
          registerSessionClaim(command, cwd, launch.sessionId);
