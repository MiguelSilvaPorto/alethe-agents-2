...
export function WorkflowDetailModal() {
  ...
  if (!active) {
    return (
      <Modal open={open} onClose={closeModal} title={t('workflow.title')} width={500}>
        <p style={{ padding: '20px 0', color: 'var(--fg-tertiary)', fontSize: 13 }}>{t('workflow.noActive')}</p>
      </Modal>
    )
  }
    ...
    <Modal open={open} onClose={closeModal} title={active.task} width={500}>
      <div className={styles.body}>
        <div className={styles.meta}>
          ...
          {active.branch && <span><GitBranch size={12} /> {active.branch}</span>}
          {active.mode === 'GIT' && branchInfo && (
            <span>{t('workflow.commits', { count: branchInfo.commitCount })}</span>
          )}
        </div>

        {active.mode === 'GIT' && branchInfo?.lastCommitMsg && (
          <p className={styles.lastCommit}>{t('workflow.lastCommit', { msg: branchInfo.lastCommitMsg })}</p>
        )}
        ...
        <div className={styles.section}>
          ...
          <div className={styles.commitRow}>
            ...
            >
              {busy === 'commit' ? <Loader2 size={12} className={styles.spin} /> : '📝'}
            </button>
          </div>
        </div>
        ...
        <div className={styles.section}>
          ...
          >
            {busy === 'complete' ? <Loader2 size={12} className={styles.spin} /> : null}
            {t('workflow.complete')}
          </button>
        </div>
      </div>
