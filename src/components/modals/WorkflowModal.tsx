...
export function WorkflowModal() {
  ...
  const handleStart = async () => {
    ...
    setError(null)
    try {
      await startSession(`wf-${Date.now()}`, agentType, task.trim(), mode, mode === 'GIT' ? repoRoot.trim() || null : null)
      closeModal()
      refresh()
    } catch (e) {
      setError(String(e))
  ...
  return (
    <Modal open={open} onClose={closeModal} title={t('workflow.new')} width={480}>
      <div className={styles.form}>
        ...
        <label className={styles.label}>
          ...
          <select className={styles.select} value={agentType} onChange={(e) => setAgentType(e.target.value)}>
            {AGENTS.map((a) => (
