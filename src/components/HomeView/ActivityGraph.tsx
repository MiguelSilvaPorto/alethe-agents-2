...
function totalAndDelta(days: ActivityDay[]): {
  total: number;
  deltaPct: number | null;
} {
  if (days.length < 14)
    return { total: days.reduce((s, d) => s + d.count, 0), deltaPct: null };
  const halfPoint = days.length - 7;
  const recent = days.slice(halfPoint).reduce((s, d) => s + d.count, 0);
  const prev = days
    .slice(halfPoint - 7, halfPoint)
      ...
      <div className={styles.cardHead}>
        ...
        <div className={styles.headRight}>
          ...
          >
            <RefreshCw
              size={12}
              className={refreshing ? styles.iconBtnSpin : undefined}
            />
          </button>
        </div>
      </div>
        ...
        {loading ? (
          ...
          <>
                  ...
                  <div key={ci} className={styles.activityCol}>
                    {col.map((day, ri) =>
                      day ? (
                        ...
                        <div
                          key={ri}
                          className={`${styles.activityCell} ${styles.cellEmpty}`}
                        />
                      ),
                    )}
                  </div>
            ...
            <span className={styles.activityLegendGroup}>
              ...
              />
              <span
                className={`${styles.activityLegendCell} ${styles.cell1}`}
              />
              <span
                className={`${styles.activityLegendCell} ${styles.cell2}`}
              />
              <span
                className={`${styles.activityLegendCell} ${styles.cell3}`}
              />
              <span
