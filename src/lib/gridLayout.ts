...
export function reconcileGridLayout(
  ...
  const normalize = (cell: GridCell | undefined): GridCell => {
    const colSpan = Math.max(
      1,
      Math.min(cols, Math.floor(cell?.colSpan ?? 1) || 1),
    );
    const rowSpan = Math.max(
      1,
      Math.min(rows, Math.floor(cell?.rowSpan ?? 1) || 1),
    );
    return {
      col: Math.max(
        1,
        Math.min(cols - colSpan + 1, Math.floor(cell?.col ?? 1) || 1),
      ),
      row: Math.max(
        1,
        Math.min(rows - rowSpan + 1, Math.floor(cell?.row ?? 1) || 1),
      ),
      colSpan,
      rowSpan,
    };
